...
class AttentionGate(nn.Module):
    def __init__(self, in_ch):
        super().__init__()
        self.conv_gate = nn.Conv2d(2*in_ch, 2*in_ch, 1, stride = 1)
        self.conv_skip = nn.Conv2d(in_ch, 2*in_ch, 1, stride = 2)
        self.relu = nn.ReLU()
        self.conv_sum = nn.Conv2d(2*in_ch, 1, 1, stride = 1)
...
class UNet(nn.Module):
    def __init__(self,in_channels=1,base_ch=32):
        ...
        #Gate
        self.gate_3 = AttentionGate(base_ch*4)
        self.gate_2 = AttentionGate(base_ch*2)
        self.gate_1 = AttentionGate(base_ch)
    ...
    def forward(self, x):
        ...
        skip3 = self.gate_3(b, skip3)
        y3 = self.up3(b, skip3)
        skip2 = self.gate_2(y3, skip2)
        y2 = self.up2(y3, skip2)
        skip1 = self.gate_1(y2, skip1)
